...
export function Passenger({ signedIn, firstName, settings, onLogin, onRefresh }: { signedIn: boolean; firstName?: string; settings: Settings | null; onLogin: () => void; onRefresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const expiry = setTimeout(() => { setQuote(null); setMessage('O valor expirou. Toque em Ver valor da corrida para atualizar.'); }, Math.max(0, remaining));
    return () => clearTimeout(expiry);
  }, [quote, busy]);
  function editPoint(kind: 'pickup' | 'destination') {
    setEditing(kind); setQuery(''); setResults([]); setSearchProvider(''); setSearched(false); setMessage('');
  }
  ...
  return <div className="passenger-grid"><section className="booking-column"><div className="page-heading"><p className="eyebrow">{firstName ? `OLÁ, ${firstName.toLocaleUpperCase('pt-BR')}` : 'SEU CAMINHO, MAIS SIMPLES'}</p><h1>Bora de moto<span>?</span></h1><p>Para onde vamos hoje?</p></div>
    <div className="booking-card"><div className="route-fields"><span className="route-line" aria-hidden="true" /><button className="place-field" disabled={busy} onClick={() => editPoint('pickup')}><span className="origin-dot" /><span><small>EMBARQUE</small><strong>{pickup?.label || 'Onde você está?'}</strong></span></button><button className="place-field destination" disabled={busy} onClick={() => editPoint('destination')}><MapPin size={20} /><span><small>DESTINO</small><strong>{destination?.label || 'Para onde você vai?'}</strong></span><Search size={20} /></button></div>
      <button className="locate-link" onClick={locate} disabled={busy}><LocateFixed size={17} />Usar minha localização</button>
      <div className="payment-options"><button className={payment === 'pix' ? 'selected' : ''} disabled={busy} onClick={() => setPayment('pix')} aria-pressed={payment === 'pix'}><QrCode size={18} />Pix</button><button className={payment === 'cash' ? 'selected' : ''} disabled={busy} onClick={() => setPayment('cash')} aria-pressed={payment === 'cash'}><Banknote size={18} />Dinheiro</button><span>Direto ao motociclista</span></div>
    ...
    {editing ? <Modal title={editing === 'pickup' ? 'Seu ponto de embarque' : 'Para onde vamos?'} onClose={() => { setEditing(null); setMessage(''); }}><form className="search-form" onSubmit={async event => {
      ...
      try { const found = await searchPlaces(query); setResults(found.results); setSearchProvider(found.provider || ''); setSearched(true); } catch (error) { setMessage((error as Error).message); } finally { setBusy(false); }
      ...
      {searchProvider === 'geoapify' ? <p className="helper">Busca: <a href="https://www.geoapify.com/" target="_blank" rel="noreferrer">Geoapify</a> · <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap</a></p> : null}
