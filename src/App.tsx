...
export default function App() {
  ...
  const [view, setView] = useState<View>('home'), [dialog, setDialog] = useState<Dialog>(null), [receipt, setReceipt] = useState<Ride | null>(null);
  ...
  return <div className="app-shell"><aside className="sidebar"><a className="brand" href="/"><span className="brand-emblem"><Bike size={24} /></span>Moto<span>Já</span></a><span className="city-pill"><MapPin size={15} />Campo Verde, MT</span><nav aria-label="Menu principal">{nav.map(({ id, label, Icon }) => <button key={id} className={view === id ? 'active' : ''} onClick={() => go(id)}><Icon size={21} />{label}{id === 'home' && active ? <span className="nav-dot" /> : null}</button>)}{role === 'admin' ? <button className={view === 'admin' ? 'active' : ''} onClick={() => go('admin')}><LayoutDashboard size={21} />Administração</button> : null}</nav><div className="sidebar-bottom"><div className="local-brand"><span>É DAQUI.</span><strong>É para você.</strong><p>Mobilidade em Campo Verde.</p></div><button className="sidebar-help" onClick={() => setDialog('safety')}><Headphones size={18} />Precisa de ajuda?<ArrowRight size={16} /></button><button className="sidebar-help" onClick={() => setDialog('install')}><Download size={18} />Instalar no celular</button><small>MotoJá · 0.2.0</small></div></aside>
    ...
    {dialog ? <Modal title={({ auth: 'Sua conta MotoJá', safety: 'Central de segurança', terms: 'Regras e privacidade', mfa: 'Proteção da conta', recovery: 'Recuperar senha', install: 'MotoJá no seu celular', notifications: 'Suas notificações' })[dialog]} onClose={() => setDialog(null)}><Suspense fallback={<p>Abrindo…</p>}>
